...
import { Button, TextFilter, Cards, Alert, Spinner, Icon, Modal, Box, SpaceBetween, Badge, ButtonDropdown } from '@cloudscape-design/components';
...
class VideoSearch extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            selectedItemId: null,
            pageSize: 8,
            mmScoreThreshold: 0.52,
            textScoreThreshold: 0.003,

            selectedSearchOptionId: "text",
            imageBytes: null,
        ...
        this.searchOptions = [
            { text: "Text only", id: "text" },
            { text: "Semantic search", id: "text_embedding" },
            { text: "Multimodal search", id: "mm_embedding"},
        ];
    }

    handleVideoClick = taskId => {
        this.props.onThumbnailClick(taskId);
    }
                ...
                {this.state.alert}
                     ...
                     <div className='searchinput'>
                        ...
                        <div className="search-by-image">
                            ...
                            <div className='desc'>Choose a similiarity score threshold</div>
                            <Slider
                                defaultValue={this.state.mmScoreThreshold}
                                aria-label="Default"
                                ...
                                max={1}
                                onChange={(e)=>{
                                    this.setState({mmScoreThreshold: e.target.value});
                                }}
                        ...
                        :this.state.selectedSearchOptionId === "text_embedding"?
                        <div className="search-by-text">
                            <div className='title'>Perform a search using text embedding</div>
                            <div className='desc'>Choose a similiarity score threshold</div>
                            <Slider
                                defaultValue={this.state.textScoreThreshold}
                                aria-label="Default"
                                valueLabelDisplay="auto"
                                step={0.001}
                                min={0}
                                max={0.01}
                                onChange={(e)=>{
                                    this.setState({textScoreThreshold: e.target.value});
                                }}
                                className="score-threshold"
                            />
                        </div>
                ...
                {this.state.items !== undefined && this.state.items !== null && this.state.items.length > 0?this.state.items.map((l,i)=>{
                    return  <div className="thumbnail" key={l.TaskId}>
                                {l.ThumbnailUrl === undefined || l.ThumbnailUrl === null? 
                                    <img className='img' src={DefaultThumbnail} alt="Generating thumbnail" onClick={({ detail }) => {this.handleVideoClick(l.TaskId);}}></img>
                                    :<img className='img' src={l.ThumbnailUrl} alt={l.FileName} onClick={({ detail }) => {this.handleVideoClick(l.TaskId);}}></img>}
                                <div className="title" onClick={({ detail }) => {this.handleVideoClick(l.TaskId);}}>{l.FileName}</div>
